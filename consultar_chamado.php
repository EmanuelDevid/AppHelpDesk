<?php require_once "validador_acesso.php";
    ...
    $chamados = array();//vetor a alocar os dados do arquivo

    $arquivo = fopen('C:\xampp\app_help_desk\arquivo.txt', 'r');//abrindo arquivo

    while(!feof($arquivo)){
        $registro = fgets($arquivo);//pegando cada linha do arquivo
        $registro_detalhes = explode('#', $registro);//dividindo as sessões de um chamado e guardando na variável em questão

        if($_SESSION['perfil_id'] == 2){//caso verdade é um usuário
            if($_SESSION['id'] != $registro_detalhes[0]){//verificando se o perfil logado foi quem abriu o chamado da iteração
                continue;
            }else{
                $chamados[] = $registro;//alocando os registros na variável chamado
            }
        }else{//neste caso temos um adm
            $chamados[] = $registro;//alocando os registros na variável chamado
        }
    }
